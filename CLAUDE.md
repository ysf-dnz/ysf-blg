...
## Online admin (Keystatic — 2026-08-31 CANLI)
- **https://yusufdeniz.com/keystatic** — GitHub ile giriş; yalnızca `ysf-dnz/ysf-blg` reposuna YAZMA yetkisi olan girebilir (ayrı şifre yok).
- Depolama `keystatic.config.tsx`'te KOŞULLU: canlıda `storage.kind="github"` (her kayıt repoya commit), lokalde `local` (offline çalışır). Bayrak `import.meta.env` ile okunur — bu dosya TARAYICIDA da çalışır, `process.env` KULLANMA (hydration'ı kırar, yaşandı).
- `astro.config.mjs`: panel `ASTRO_KEYSTATIC=1` (lokal) VEYA `KEYSTATIC_GITHUB_CLIENT_ID` (canlı) varsa derlenir; anahtar yoksa hiç derlenmez.
- Vercel env (production): `KEYSTATIC_GITHUB_CLIENT_ID`, `KEYSTATIC_GITHUB_CLIENT_SECRET`, `KEYSTATIC_SECRET`. Değerler `.env`'de (repoda DEĞİL).
- **Vercel artık GitHub'a BAĞLI** (`vercel git connect`): Keystatic'ten kaydet → repoya commit → otomatik deploy. Elle deploy hâlâ `vercel deploy --prod --archive=tgz`.
- Kurulum tuzağı: Keystatic setup formunun "Create GitHub App" submit'i sessizce takılabiliyor; manifest'i elle POST etmek çözdü (GitHub sudo/passkey doğrulaması KULLANICIDA).
